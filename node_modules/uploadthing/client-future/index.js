...
import { createDeferred, createUTReporter, generateTraceHeaders } from "../dist/ut-reporter-C8446w-v.js";
import * as Arr$1 from "effect/Array";
...
* @public
*/
var XHRError = class extends Micro$1.TaggedError("XHRError") {};
/**
* Error indicating the upload was rejected during upload to the storage provider
* @public
*/
var UTStorageError = class extends Micro$1.TaggedError("UTStorageError") {};
/**
* Error indicating the request to your UploadThing server failed
* @public
*/
var UTServerError = class extends Micro$1.TaggedError("UTServerError") {};
/**
...
function uploadFile(url, { file, files, XHRImpl,...options }) {
	return fetchEff(url, {
	...
	}).pipe(Micro$1.map(({ headers }) => Number.parseInt(headers.get("x-ut-range-start") ?? "0")), Micro$1.map((rangeStart) => transitionToUploading(file, rangeStart)), Micro$1.tap((uploadingFile) => {
		options.onEvent({
			type: "upload-started",
			file: uploadingFile,
			files
		});
	}), Micro$1.flatMap((uploadingFile) => Micro$1.async((resume) => {
		...
		xhr.addEventListener("load", () => {
			if (xhr.status > 299 || Predicate.hasProperty(xhr.response, "error")) resume(new UTStorageError({
				...
				resume(Micro$1.succeed(uploadedFile));
			}
		});
		...
		*/
		...
		xhr.send(formData);
		return Micro$1.sync(() => xhr.abort());
	})), Micro$1.catchAll((error) => {
		...
		return Micro$1.succeed(failedFile);
	}));
}
/**
...
function requestPresignedUrls(options) {
	...
	}).pipe(Micro$1.mapError((error) => new UTServerError({
...
function uploadFiles(endpoint, options) {
	...
	return requestPresignedUrls({
		...
		package: options.package,
		traceHeaders
	}).pipe(Micro$1.map(Arr$1.zip(pendingFiles)), Micro$1.tap((pairs) => {
	...
	}), Micro$1.flatMap((pairs) => Micro$1.forEach(pairs, ([presigned, file]) => uploadFile(presigned.url, {
		file,
		files: pendingFiles,
		input: options.input,
